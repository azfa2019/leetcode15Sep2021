...
public:
    vector<int> cheapestJump(vector<int>& coins, int maxJump) {
      //dp[i]: min cost when starting from i to reach the end
      //dp[i]=min(dp[i],dp[i+x]+coins[i]) for all x: 1:maxJump
      int n=coins.size();
      vector<int>dp(n,INT_MAX/2);
      vector<int>path(n,-1);
      //path[i]: the next index position when starting from index i
      dp[n-1]=coins[n-1];
      for(int i=n-1;i>=0;i--){
      ...
      int i=0;
      while(i<coins.size()){
        ans.push_back(i+1);//when index=0, ouput 1 (1 based)
