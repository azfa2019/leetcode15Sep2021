...
public:
    ...
    vector<vector<int>> palindromePairs(vector<string>& words) {
        ...
        for(int i=0;i<words.size();i++){
            ...
            for(int j=n;j>=1;j--) b.push_back(b.back()*P+words[i][j-1]);
            h1.push_back(a);
            h2.push_back(b);
        }
        ...
        for(int i=0;i<words.size();i++){
            ...
            for(int j=0;j<=n;j++){
                ...
                if(left==get(h2[i],n-j+1,n) && hash.count(right) && hash[right]!=i && words[i].size()!=words[hash[right]].size()) res.push_back({hash[right],i});
            }
        }
        return res;
    }
